package com.paymybuddy.buddy.constants;

/**
 * @author Yahia CHERIFI
 * This class groups different constants that define database columns' lenghth
 */
public final class ConstantNumbers {

    /**
     * Class constructor.
     */
    protected ConstantNumbers() {
    }

    /**
     * Refers to the percentage that will be paid by users for each transaction.
     * Fee is paid only by users who send money
     */
    public static final Double FEE_PERCENTAGE = 0.005;

    /**
     * Refers to max characters allowed in some columns: 5.
     */
    public static final int FIVE = 5;

    /**
     * Refers to max characters allowed in some columns: 10.
     */
    public static final int TEN = 10;

    /**
     * Refers to max characters allowed in some columns: 15.
     */
    public static final int FIFTEEN = 15;

    /**
     * Refers to max characters allowed in some columns: 25.
     */
    public static final int TWENTY_FIVE = 25;

    /**
     * Refers to max characters allowed in some columns: 34.
     */
    public static final int THIRTY_FOUR = 34;

    /**
     * Refers to max characters allowed in some columns: 45.
     */
    public static final int FORTY_FIVE = 45;

    /**
     * Refers to max characters allowed in some columns: 60.
     */
    public static final int SIXTY = 60;

    /**
     * Refers to max characters allowed in some columns: 80.
     */
    public static final int EIGHTY = 80;

    /**
     * Refers to the max characters allowed in transaction description.
     */
    public static final int MAX_DESCRIPTION_CHARACTERS = 200;

    /**
     * Refers to the max amount that a user can transfer.
     */
    public static final int MAX_MONEY_ALLOWED = 1000;


}
